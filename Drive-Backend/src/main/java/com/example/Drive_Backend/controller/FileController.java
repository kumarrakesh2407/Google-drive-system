package com.example.Drive_Backend.controller;


import com.example.Drive_Backend.entity.FileEntity;
import com.example.Drive_Backend.services.FileServiceStorage;
...
public class FileController {
                                             ...
                                             @RequestParam(value = "parentFolder",required = false) Long parentFolderId)
    {
        try {
            ...
            return ResponseEntity.status(500).body("Failed upload failed! ");
        }
    }
    ...
    try{
        //will take path mysql
        FileEntity fileEntity = fileServiceStorage.getFileById(id);

        //will make path and delete file
        Path path = Paths.get(fileEntity.getPath());
